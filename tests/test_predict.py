...
THRESHOLDS = {
    "terminate_cpu": 5, "downsize_cpu": 20, "review_cpu": 50,
    "terminate_mem": 10, "downsize_mem": 30, "review_mem": 60,
}
MARGINS = {
...
def test_recommend_memory_escalates_terminate():
    """CPU says terminate, but high memory escalates to keep."""
    action = recommend(pred_high=3.0, pred_mid=1.0, thresholds=THRESHOLDS,
                       mem_high=80.0, mem_mid=75.0)
    assert action == "keep"


def test_recommend_memory_escalates_downsize():
    """CPU says downsize, but moderate memory escalates to review."""
    action = recommend(pred_high=15.0, pred_mid=10.0, thresholds=THRESHOLDS,
                       mem_high=50.0, mem_mid=40.0)
    assert action == "review"


def test_recommend_memory_no_escalation():
    """Both CPU and memory say terminate — no escalation."""
    action = recommend(pred_high=3.0, pred_mid=1.0, thresholds=THRESHOLDS,
                       mem_high=5.0, mem_mid=3.0)
    assert action == "terminate"


def test_recommend_no_memory_data():
    """Without memory predictions, CPU-only recommendation applies."""
    action = recommend(pred_high=3.0, pred_mid=1.0, thresholds=THRESHOLDS)
    assert action == "terminate"
