...
def page_recommendations():
    ...
    with col1:
        ...
        st.plotly_chart(fig_act, use_container_width=True, config={"displayModeBar": False, "scrollZoom": False})
    ...
    with col2:
        ...
        st.plotly_chart(fig_risk, use_container_width=True, config={"displayModeBar": False, "scrollZoom": False})
...
assume on-demand pricing, not reserved instances or savings plans.
    ...
    st.plotly_chart(fig_sens, use_container_width=True, config={"displayModeBar": False, "scrollZoom": False})
    ...
    has_mem = "mem_pred_mid" in rec.columns and rec["mem_pred_mid"].notna().any()
    if has_mem:
        st.markdown('<p class="section-header">Memory-Aware Safety Net</p>',
                    unsafe_allow_html=True)
        st.markdown("""
        <p class="story-text">
            Recommendations now consider <strong>both CPU and memory</strong>.
            A VM with low CPU but high predicted memory is protected from
            termination — the more conservative action wins.
        </p>
        """, unsafe_allow_html=True)

        mem_escalated = rec[
            (rec["pred_high"] < rec.get("mem_pred_high", 0)) &
            (rec["mem_pred_high"].notna())
        ]
        mc1, mc2, mc3 = st.columns(3)
        with mc1:
            mem_with_data = rec["mem_pred_mid"].notna().sum()
            kpi_card(f"{mem_with_data:,}", "VMs with Memory Model",
                     "Scored on both CPU and memory", TEXT)
        with mc2:
            high_mem = (rec["mem_pred_high"] > 60).sum() if "mem_pred_high" in rec.columns else 0
            kpi_card(f"{high_mem:,}", "High Memory (>60%)",
                     "Protected from CPU-only termination", AMBER)
        with mc3:
            keep_count = (rec["action"] == "keep").sum()
            kpi_card(f"{keep_count:,}", "VMs Kept",
                     "Safe to keep running (CPU + memory)", GREEN)
    ...
    col_config = {
        ...
        "pred_low": st.column_config.NumberColumn("CPU Low", format="%.1f%%"),
        "pred_mid": st.column_config.NumberColumn("CPU Mid", format="%.1f%%"),
        "pred_high": st.column_config.NumberColumn("CPU High", format="%.1f%%"),
        "monthly_savings": st.column_config.NumberColumn("Savings", format="$%.2f"),
    }
    if has_mem:
        col_config.update({
            "actual_mem": st.column_config.NumberColumn("Actual Mem", format="%.1f%%"),
            "mem_pred_low": st.column_config.NumberColumn("Mem Low", format="%.1f%%"),
            "mem_pred_mid": st.column_config.NumberColumn("Mem Mid", format="%.1f%%"),
            "mem_pred_high": st.column_config.NumberColumn("Mem High", format="%.1f%%"),
        })
    st.dataframe(
        filtered,
