...
def build_daily_series(zf: zipfile.ZipFile, filename: str) -> dict[str, dict[str, float]]:
    """Return {instance: {date_str: mean_value}} from a vrops CSV.

    Each CSV row is one observation for one VM on one day.  Multiple
    observations per (instance, date) are averaged.
    """
    from collections import defaultdict
    ...
    with zf.open(PREFIX + filename) as f:
        ...
        for row in reader:
            try:
                ...
                val = float(row["Value"])
                date = row["Date"][:10]
                buckets[inst][date].append(val)
            except (ValueError, KeyError):
                continue

    result: dict[str, dict[str, float]] = {}
    for inst, dates in buckets.items():
        result[inst] = {d: sum(vs) / len(vs) for d, vs in dates.items()}
    return result


def write_daily_csv(daily: dict[str, dict[str, float]], out_path: Path) -> None:
    """Write a wide-format daily CSV: instance, date1, date2, ..."""
    all_dates = sorted({d for dates in daily.values() for d in dates})
    with open(out_path, "w", newline="\n") as f:
        w = csv.writer(f)
        w.writerow(["instance"] + all_dates)
        for inst in sorted(daily):
            row = [daily[inst].get(d, "") for d in all_dates]
            w.writerow([inst] + row)
    logger.info("Wrote %s (%d VMs, %d days)", out_path, len(daily), len(all_dates))
...
def build(out_dir: Path | None = None) -> tuple[Path, Path]:
    ...
    with zipfile.ZipFile(ZIP) as zf:
        ...
        logger.info("Building daily CPU time series...")
        cpu_daily = build_daily_series(zf, "vrops_virtualmachine_cpu_usage_ratio_all.csv")
        write_daily_csv(cpu_daily, out_dir / "vm_cpu_daily.csv")

        logger.info("Building daily memory time series...")
        mem_daily = build_daily_series(zf, "vrops_virtualmachine_memory_usage_ratio_all.csv")
        write_daily_csv(mem_daily, out_dir / "vm_mem_daily.csv")
