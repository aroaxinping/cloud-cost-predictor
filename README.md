...
**[Pitch deck](https://aroaxinping.com/cloud-cost-predictor/pitch.html)**
...
Two parallel XGBoost quantile regression models predict CPU and memory utilization independently. Recommendations combine both: the more conservative action wins, so a VM with low CPU but high memory is protected from termination.

### CPU model

Trained on 25 days of daily time series to predict next-week utilization.
...
Trained on summary-derived features (same architecture as CPU) to predict memory utilization bounds.

| Metric | Value |
|---|---|
| Algorithm | XGBoost quantile regression (q=0.10, 0.50, 0.95) |
| Median model loss | Asymmetric (3x penalty on underpredictions) |
| Features | 5 (mem_std, mem_min, mem_p50, mem_trend, mem_cv) |
| Median model Test MAE | 0.31% |
| 85% prediction interval coverage | 87.3% |
| Trained on | 123K VMs (80/20 split) |

When daily memory time series becomes available (via re-ingest), the memory model can be retrained with a proper temporal split matching the CPU model.
...
| Action | VMs (CPU+Mem) | Savings potential |
|---|---|---|
| Terminate | 502 | conservative |
| Downsize | 20,174 | moderate |
| Review | 18,619 | investigation needed |
| Keep | 84,068 | n/a |

Memory integration shifted recommendations significantly: 103K VMs with low CPU but high memory (fleet median memory: 87%) are now protected from aggressive action.
...
Interactive Streamlit app with five pages.
...
Fleet waste breakdown with real EC2 pricing, model explainer with quantile bands and SHAP waterfalls, recommendations explorer with risk levels, a Reserved Instances page comparing on-demand vs RI costs for "keep" VMs, and a "Try It" page for scoring your own VMs.
...
notebooks/
  ...
  07_memory_model.ipynb     <- memory quantile model training
src/
  ingest.py           <- stream SAP zip to per-VM summaries + daily time series
  eda.py              <- classify VMs (config-driven, memory-aware)
  pricing.py          <- map to EC2 pricing, estimate waste
  memory_model.py     <- memory quantile prediction model
  predict.py          <- load models, generate CPU+memory recommendations
...
- **Memory model uses summary-only features.** Without daily memory time series, the memory model trains on summary statistics (no temporal split). When raw data is re-ingested, it should be retrained with the same temporal approach as the CPU model.
...
- ~~**Memory quantile predictions**~~ ✅ Done — parallel memory model trained, integrated into recommendations
